...
//   Init_SiShu();
   ...
   pos_ctrl.umin=-UMAX;
   pos_ctrl.Ts=0.002;
   pos_ctrl.b=600;  // b越大，超调越小，运行越平稳	  //模型参数
   pos_ctrl.a=-3;  // a<=0;
   pos_ctrl.zeta=0.8;  //线性控制的阻尼系数
   pos_ctrl.omega=40;    //线性控制的自然频率
   pos_ctrl.zeta0=0.7;  //观测器极点的阻尼系数 0.7071
   pos_ctrl.omega0=100;    //观测器极点的自然频率
   pos_ctrl.fd=0;    //扰动补偿的稳态系数
   pos_ctrl.alpha=10;    //非线性函数的参数
   pos_ctrl.beta=0;    //非线性函数的参数
...
//		DirectionQep = EQep1Regs.QEPSTS.bit.QDF;
		...
		MechTheta = PosInit - (float)PosRevScale*(float)RawTheta;
...
{ 
	...
	PosInRev = PosRevCnt + PosInit - MechTheta;
