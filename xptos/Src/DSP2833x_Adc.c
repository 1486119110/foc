...
{	
    ...
    while (AdcRegs.ADCST.bit.SEQ1_BSY == 1)
     ...
     //特定运行状态下对HALL赋值-------霍尔电流传感器零漂补偿值
     if(Run_PMSM==2)
     {
          HALL_V=AD_BUF[11]+AD_BUF[10]+AD_BUF[9]+AD_BUF[8];//
          HALL_V=HALL_V>>2;     //>> 2 相当于除以4，得到平均值
...
//0.010986328125=0.732421875*0.01*1.5  1mv 0.01安 ------------将 ADC 码值转换为物理电流（单位：安）的换算系数
     ic=_IQdiv(_IQmpy((_IQ(ADC_U)-_IQ(HALL_U)),_IQ(0.010986328125)),_IQ(E_Ding_DianLiu));//MV转换为IQ格式，先做乘法在做除法
...
void DC_Link(void) // 计算母线电压
{
    ...
    U_dc = U_dc * 0.732421875 / 2.0; // 12位ADC的量化电压1LSB=3/pow(2,12)
