...
{
	int16	i;
	Uint32 *Source = (void *) &PieVectTableInit;            //默认中断向量表
	Uint32 *Dest = (void *) &PieVectTable;                  //实际使用的中断向量表地址
		
	EALLOW;	
	for(i=0; i < 128; i++)
		*Dest++ = *Source++;	                            //把默认中断向量表 PieVectTableInit 中的内容逐个拷贝到 PieVectTable
	EDIS;

	// Enable the PIE Vector Table
	PieCtrlRegs.PIECTRL.bit.ENPIE = 1;	                    //启用 PIE 控制器
