...
void  QEP_Init(void)
{
        ...
        EQep1Regs.QDECCTL.all=0x400;	   // Quadrature count, align with xptos
        EQep1Regs.QEPCTL.all=0x900a;	   // Unit timeout latch + enable
        EQep1Regs.QPOSCNT=0;
        EQep1Regs.QPOSINIT=0;
        EQep1Regs.QPOSMAX=(Uint16)MaxPulses;	// 1000-line encoder -> 4000 counts
...
static inline void UpdatePositionFeedback(Uint32 new_theta)
{
	const Uint16 wrap_margin = (Uint16)(MaxPulses / 40U);  // 4000/40 = 100, match xptos window
	const Uint16 forward_high = (Uint16)(MaxPulses - wrap_margin);
	const Uint16 forward_low = (Uint16)(((MaxPulses / 4U) > wrap_margin)
	                                   ? ((MaxPulses / 4U) - wrap_margin)
	                                   : 0U);
	const Uint16 reverse_high = (Uint16)((MaxPulses * 3U) / 4U);
	const Uint16 reverse_low = (Uint16)(MaxPulses / 4U);
	...
	if(direction == 1U)
	{
		if((OldRawTheta > forward_high) && (new_theta < forward_low))
		{
	...
	{
		if((new_theta > reverse_high) && (OldRawTheta < reverse_low))
		{
			delta -= (float32)MaxPulses;
			PosRevCnt--;
		}
