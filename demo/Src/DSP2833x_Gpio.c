...
void InitGpio(void)
{
   ...
   GpioCtrlRegs.GPCPUD.all = 0x0000;      // Pullup's enabled GPIO64-GPIO79
    ...
    GpioDataRegs.GPADAT.bit.GPIO15=1;//DC_ON ߵƽ,Ϊ͵ƽʱ ĸ߼̵պ
    ...
    GpioDataRegs.GPADAT.bit.GPIO20=1;// SYNCߵƽ 
    GpioCtrlRegs.GPBMUX2.bit.GPIO50=1;//  a
    GpioCtrlRegs.GPBMUX2.bit.GPIO51=1;//  b
     GpioCtrlRegs.GPBMUX2.bit.GPIO53=1;//  c

      GpioIntRegs.GPIOXINT3SEL.bit.GPIOSEL=53;//gpio53 Ϊⲿж3
    XIntruptRegs.XINT3CR.bit.ENABLE=1;//ⲿжϴ
...
void  IPM_BaoHu(void)
{
    ...
    j=GpioDataRegs.GPADAT.bit.GPIO13;//ȡλ
    if(j==0)
    {
        i++;
        if(i==1)
